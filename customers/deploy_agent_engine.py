...
def get_requirements_from_uv():
    ...
    result = subprocess.run(
        f"uv export --format requirements-txt --no-hashes --no-dev --package customers --no-emit-workspace | grep -v '#'",
        shell=True,
        capture_output=True,
        text=True,
        check=True,
...
def main():
    ...
    # 1. Manually refresh credentials to solve the Cloud Shell "default" identity issue
    ...
    # Initialize with refreshed credentials
    vertexai.init(
        project=project_id, 
        location=location, 
    ...
    existing_resource_name = os.environ.get("CUSTOMERS_ENGINE_ID")
    
    if existing_resource_name:
        print(f"Using CUSTOMERS_ENGINE_ID from environment: {existing_resource_name}")
    else:
        ...
        try:
            ...
            print("If you know your Engine ID, set it in CUSTOMERS_ENGINE_ID in config.sh to skip this check.")
